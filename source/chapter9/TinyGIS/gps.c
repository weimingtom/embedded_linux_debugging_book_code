/*
 * $file	:	gps.c
 * $desc	:	implement GPS supporting for TinyGIS
 * $author	:	rockins
 * $date	:	Thu Apr  5 02:08:27 CST 2007
...
// is defined as empty, which do nothing
...
#elif defined(X86_PC)
	#define	SWAP_DOUBLE(x)	x = x
#endif

MAPNODE TopLeft;					// topleft benchmark
MAPNODE BottomRight;				// bottomright benchmark
MAPNODE CurrentLocation;			// current location
pthread_t gps_thread;				// identifier of gps thread
pthread_mutex_t GPSMutex = 
	PTHREAD_MUTEX_INITIALIZER;		// mutex for access CurrentLocation
int isSampling = 0;					// indicating sampling or not, 
									// 0 means not, 1 mean yes
								...
								double *latitude)
{
	...
	pos = strstr(gps_data, GPGGA_HEAD);
	assert(pos != NULL);
...
//
int 
GPS_GetCurrentLonLat(double *lon, double *lat)
{
	if (!isSampling)
		return (-1);
	...
	*lat = CurrentLocation.latitude;
	...
	assert(*lon > DBL_EPSILON);
	assert(*lat > DBL_EPSILON);
...
//
int
GPS_StopSampleProcess(void)
{
	int err = 0;

	err = pthread_cancel(gps_thread);
	return (err);
}
