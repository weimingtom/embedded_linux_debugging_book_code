/*
 * $file	:	gui.h
 * $desc	:	graphical user interface definition
 * $author	:	rockins(ybc2084@163.com)
 * $date	:	Thu Apr  5 02:13:40 CST 2007
...
#ifndef	_GUI_DEBUG_
//#define	_GUI_DEBUG_							// for debugging
#endif
...
#define	IDC_GPS_TIMER	400						// gps sample timer


#define	GPS_SAMPLE_TIME_UNIT	200				// 200 * 10ms = 2s

typedef struct _RASTER_MAP
{
#define	MAP_FILE_LEN	100						// map file path len
	char	*bmpname;							// .bmp file name
	BITMAP	map;								// map object, in bitmap format
	unsigned int tw, th;						// total width and height of map
	unsigned int lx, ty;						// left x and top y of map
	unsigned int dw, dh;						// display width and height of map
	unsigned int vx, vy;						// vehicle x and y in map
}RASTER_MAP;
